  ...
  return (
    <SafeAreaProvider initialMetrics={initialWindowMetrics}>
      <SafeAreaView style={styles.container}>
        {!isReady && <Text style={styles.helperText}>Carregando dados salvos…</Text>}
        <StatusBar style="dark" />
        <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View pointerEvents={sending || !isReady ? 'none' : 'auto'} style={styles.header}>
          ...
          <View pointerEvents={sending || !isReady ? 'none' : 'auto'} style={styles.navBackground}><View style={styles.bottomNav}>
            {TABS.map((tab) => <Pressable key={tab.screen} accessibilityRole="tab" accessibilityLabel={tab.title} accessibilityState={{ selected: screen === tab.screen }} onPress={() => router.navigate(ROUTES[tab.screen])} style={({ pressed }) => [styles.bottomNavButton, screen === tab.screen && styles.activeBottomNavButton, pressed && styles.pressed]}>
